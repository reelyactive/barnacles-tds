...
function handleConnectionEvents(instance) {
  instance.connection.on('connect', (error) => {
    if(error) {
      if(instance.printErrors) {
        console.log('barnacles-tds: Database connection failed with', error);
      }
    }
    else {
      if(instance.printErrors) {
...
function handleRaddec(instance, raddec) {
  const filter = new RaddecFilter(instance.eventsToStore?.raddec?.filters ||
                                  DEFAULT_RADDEC_FILTERS);
