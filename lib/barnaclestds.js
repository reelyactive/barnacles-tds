...
const DEFAULT_MAX_REQUEST_QUEUE_LENGTH = 1000;
...
class BarnaclesTDS extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.maxRequestQueueLength = options.maxRequestQueueLength ||
                                 DEFAULT_MAX_REQUEST_QUEUE_LENGTH;
...
function handleRaddec(instance, raddec) {
  ...
  if(raddecOptions.filter.isPassing(raddec)) {
    ...
    const event = { name: 'raddec', data: raddec };

    enqueueTdsRequest(instance, requestString, event);
  }
}
...
function handleDynamb(instance, dynamb) {
  ...
  if(dynambOptions.filter.isPassing(dynamb)) {
    ...
    const event = { name: 'dynamb', data: dynamb };
 ...
 * @param {BarnaclesTDS} instance The BarnaclesTDS instance.
 * @param {String} requestString The request as a string.
 * @param {Object} event The event name and data.
 */
function enqueueTdsRequest(instance, requestString, event) {
  if(instance.tdsRequestQueue.length < instance.maxRequestQueueLength) {
    ...
    // TODO: log when tdsRequestQueue overflows?
  }
}
